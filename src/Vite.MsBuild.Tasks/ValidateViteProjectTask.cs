    ...
    public class ValidateViteProjectTask : Task
    {
        ...
        public override bool Execute()
        {
            try
            {
                ...
                if (IsValid)
                {
                    Log.LogMessage(MessageImportance.Normal, "✅ Vite project validation completed successfully");
                }
        ...
        private void ValidatePackageJson()
        {
            ...
            if (!HasPackageJson)
            {
                Log.LogError($"❌ No package.json found in {ViteProjectRoot}. Run 'npm init' to create one.");
                return;
            }

            Log.LogMessage(MessageImportance.Low, $"✅ Found package.json: {packageJsonPath}");
            ...
            try
            {
                ...
                {
                    Log.LogWarning("⚠️ package.json appears to be invalid or empty");
                }
            }
            catch (Exception ex)
            {
                Log.LogWarning($"⚠️ Could not read package.json: {ex.Message}");
            }
        }
        ...
        private void ValidateViteConfig()
        {
            ...
            if (!string.IsNullOrEmpty(ViteConfigFile))
            {
                ...
                if (File.Exists(userConfigPath))
                {
                    ...
                    Log.LogMessage(MessageImportance.Low, $"✅ Found user-specified Vite config: {ViteConfigFile}");
                }
                else
                {
                    Log.LogWarning($"⚠️ User-specified Vite config not found: {ViteConfigFile}");
                }
            }
            ...
            if (foundConfig == null)
            {
                foreach (var configName in possibleConfigs)
                {
                    ...
                    if (File.Exists(configPath))
                    {
                        ...
                        Log.LogMessage(MessageImportance.Low, $"✅ Auto-detected Vite config: {configName}");
                        break;
                    }
                }
            }
        ...
        private void ValidateNodeModules()
        {
            ...
            {
                Log.LogMessage(MessageImportance.Normal, 
                    $"📦 node_modules not found. Will run '{PackageManager} install' during build.");
            }
            else
            {
                Log.LogMessage(MessageImportance.Low, "✅ node_modules directory found");
                
                // Check for Vite dependency
                var vitePath = Path.Combine(nodeModulesPath, "vite");
                if (Directory.Exists(vitePath))
                {
                    Log.LogMessage(MessageImportance.Low, "✅ Vite dependency found in node_modules");
                }
                else
                {
                    Log.LogMessage(MessageImportance.Normal, 
                        "💡 Vite not found in node_modules. Ensure it's listed in your dependencies.");
                }
            }
        }
        ...
        private void ValidateEnvironmentFiles()
        {
            ...
            foreach (var envFile in CommonEnvFiles)
            {
                ...
                {
                    Log.LogMessage(MessageImportance.Low, $"✅ Found environment file: {envFile}");
                }
            }
            ...
            if (MissingEnvFiles.Length > 0)
            {
                ...
                {
                    case "error":
                        Log.LogError($"❌ Missing environment files: {missingList}");
                        break;
                    case "warn":
                        Log.LogWarning($"⚠️ Missing environment files: {missingList}");
                        break;
                    case "silent":
                    default:
                        Log.LogMessage(MessageImportance.Low, $"💡 Optional environment files not found: {missingList}");
                        break;
                }
            }
        }
        ...
        private void ValidatePackageManager()
        {
            ...
            {
                Log.LogWarning($"⚠️ Unsupported package manager '{PackageManager}'. Supported: {string.Join(", ", supportedManagers)}");
            }
            else
            {
                Log.LogMessage(MessageImportance.Low, $"✅ Using package manager: {PackageManager}");
            }
        }
        ...
        private void ShowWelcomeMessageOnce()
        {
            ...
            if (HasPackageJson && HasViteConfig && !HasNodeModules)
            {
                ...
                Log.LogMessage(MessageImportance.High, "🎉 Welcome to Vite.MsBuild!");
                Log.LogMessage(MessageImportance.High, "");
                Log.LogMessage(MessageImportance.High, "✅ Your project is configured for automatic Vite builds");
                Log.LogMessage(MessageImportance.High, $"📦 Package manager: {PackageManager}");
                
                if (HasViteConfig)
                {
                    Log.LogMessage(MessageImportance.High, $"⚙️  Vite config: Found");
                }
