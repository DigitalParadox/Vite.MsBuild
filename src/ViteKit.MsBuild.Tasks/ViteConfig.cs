...
namespace ViteKit.MsBuild.Tasks
{
    ...
    public class ViteConfig : Microsoft.Build.Utilities.Task
    {
        ...
        public override bool Execute()
        {
            try
            {
                Log.LogMessage(MessageImportance.Low, "[BUILD] Defining Vite config: {0} -> {1}", Name, ConfigFile);
                ...
                Log.LogMessage(MessageImportance.Normal, "[OK] Vite config '{0}' defined successfully", Name);
                return true;
            }
            catch (Exception ex)
            {
