...
namespace ViteKit.MsBuild.Tasks
{
    ...
    {
        // Static cache for config validation results (survives across builds in same process)
        private static readonly Dictionary<string, (DateTime lastModified, bool isValid, string? outputDir, string[] warnings)> _configCache = [];
        ...
        public override bool Execute()
        {
            try
            {
                ...
                if (_configCache.TryGetValue(ConfigPath, out var cached))
                {
                    ...
                    {
                        Log.LogMessage(MessageImportance.Low, "⚡ Using cached config validation result");
                        IsValid = cached.isValid;
                        DetectedOutputDir = cached.outputDir;
                ...
                {
                    Log.LogMessage(MessageImportance.Low, "[OK] Config file appears to have valid export syntax");
                }
                else
                {
                ...
                _configCache[ConfigPath] = (lastModified, IsValid, DetectedOutputDir, Warnings);
