    ...
    public class CustomCommandBuilderTests : IDisposable
    {
        ...
        public void CustomCommandBuilder_Should_Handle_Complex_Commands()
        {
            ...
            var config = new ViteBuildConfiguration
            {
                ProjectRoot = _tempDirectory,
                PackageManager = PackageManager.Npm,
                CustomBuildCommand = "pnpm dlx vite build --config custom.config.ts --mode staging"
            };
            var command = new ViteCommandBuilder(config).BuildCommand();
        ...
        public void CustomCommandBuilder_Should_Throw_For_Empty_Command()
        {
            ...
            var config = new ViteBuildConfiguration
            {
                ProjectRoot = _tempDirectory,
                PackageManager = PackageManager.Npm,
                CustomBuildCommand = "   " // Use whitespace instead of empty string
            };

            // Assert: Should throw for empty/whitespace command
            var exception = Assert.Throws<ArgumentException>(() => new ViteCommandBuilder(config).BuildCommand());
